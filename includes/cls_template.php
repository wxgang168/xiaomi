...
class cls_template
{
    ...
    function fetch_str($source)
    {
        ...
        return preg_replace_callback(
            "/{([^\}\{\n]*)}/",
            function($preg_match) use($that) { return $that->select($preg_match[1]); },
            $source);
    }
            ...
            switch ($tag_sel)
            {
                ...
                case 'insert' :
                    ...
                    $out = "<?php \n" . '$k = ' . preg_replace_callback(
                        "/(\'\\$[^,]+)/" ,
                        function ($preg_match) { return stripslashes(trim($preg_match[1],'\''));},
                        var_export($t, true)) . ";\n";
    ...
    function get_val($val)
    {
        if (strrpos($val, '[') !== false)
        {
            ...
            $val = preg_replace_callback(
                "/\[([^\[\]]*)\]/is",
                function ($preg_match) { return '.'.str_replace('$', '\$', $preg_match[1]); },
                $val);
        }
    ...
    function smarty_prefilter_preCompile($source)
    {
        ...
        if ($file_type == '.dwt')
        {
            ...
            $source      = preg_replace_callback(
                ...
                function ($preg_match) { return '{include file='.strtolower($preg_match[1]). '}' ;},
                $source);

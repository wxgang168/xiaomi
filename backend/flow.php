...
elseif ($_REQUEST['step'] == 'add_to_cart_combo')
{
    include_once('includes/cls_json.php');
    $_POST['goods']=strip_tags(urldecode($_POST['goods']));
    $_POST['goods'] = json_str_iconv($_POST['goods']);
...
elseif ($_REQUEST['step'] == 'del_in_cart_combo')
{
    include_once('includes/cls_json.php');
    $_POST['goods']=strip_tags(urldecode($_POST['goods']));
    $_POST['goods'] = json_str_iconv($_POST['goods']);
...
elseif ($_REQUEST['step'] == 'add_cart_combo_list') {
    include_once('includes/cls_json.php');
...
elseif ($_REQUEST['step'] == 'add_del_cart_combo_list')
{
    include_once('includes/cls_json.php');
    $_POST['group']=strip_tags(urldecode($_POST['group']));
    $_POST['group'] = json_str_iconv($_POST['group']);
...
{ 
    ...
    if($pay_online['pay_online'] || ($pay_online['user_surplus'] && $user_info['user_money'] > 0)) {
...
elseif ($_REQUEST['step'] == 'done')
{
    ...
    if($_REQUEST['act'] == 'chunsejinrong'){
        ...
        $done_cart_value = isset($_POST['done_cart_value']) ? dsc_addslashes($_POST['done_cart_value']) : 0; //取得购买购物车商品ID
        ...
        $order = array(
            'shipping_id' => $shipping['shipping_id'], //旺旺ecshop2012--zuo
            'shipping_type' => $shipping['shipping_type'], //旺旺ecshop2012--zuo
        ...
        if ($cartValue && $is_done == 0) {
            ...
            if (file_exists(MOBILE_DRP)) {
                ...
                $sql = "INSERT INTO " . $ecs->table('order_goods') . "( " .
                        "user_id, cart_recid, goods_id, goods_name, goods_sn, product_id, goods_number, market_price, goods_price, stages_qishu," .
                        ...
                        " SELECT '$user_id', '$cartValue', ca.goods_id, ca.goods_name, ca.goods_sn, ca.product_id, ca.goods_number, ca.market_price, ca.goods_price, ca.stages_qishu, ca.goods_attr, " .
                ...
                /* 插入订单商品 */
                $sql = "INSERT INTO " . $ecs->table('order_goods') . "( " .
                        "user_id, cart_recid, goods_id, goods_name, goods_sn, product_id, goods_number, market_price, commission_rate, stages_qishu, " .
                        ...
                        " SELECT '$user_id', '$cartValue', goods_id, goods_name, goods_sn, product_id, goods_number, market_price, commission_rate, stages_qishu, " .
                        "goods_price, goods_attr, is_real, extension_code, parent_id, is_gift, model_attr, goods_attr_id, ru_id, shopping_fee, warehouse_id, area_id, freight, tid, shipping_fee" .
                        " FROM " . $ecs->table('cart') .
        ...
        if ($new_order_id > 0 && $_CFG['use_storage'] == '1' && $_CFG['stock_dec_time'] == SDT_PLACE) {
    ...
    if (isset($_POST['rec_id']) && isset($_POST['goods_number'])) {
        ...
        if ($val > 0) {
            ...
            else 
            {
                ...
                $goods_price = get_final_price($row['goods_id'], $val, true, $attr_id, $_POST['warehouse_id'], $_POST['area_id'], 0, 0, $add_tocart); //旺旺ecshop2012--zuo
                //更新购物车中的商品数量
                $set = "";
                if($goods_price > 0){

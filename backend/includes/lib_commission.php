    ...
    foreach ($row as $key => $value)
    {
        ...
        if(empty($value['chargeoff_status'])){
            ...
            if ($value['chargeoff_status'] == 1) {
                $where_order = " AND " . db_create_in(array(OS_UNCONFIRMED), "o.shipping_status") .
                        " AND " . db_create_in(array(PS_UNPAYED), "o.pay_status") .
                        " AND " . db_create_in(array(OS_RETURNED), "o.order_status");

                $sql = "SELECT SUM(sbo.return_amount) AS return_amount, SUM(sbo.return_shippingfee) AS return_shippingfee FROM " . $GLOBALS['ecs']->table('seller_bill_order') . " AS sbo " .
                        "LEFT JOIN " . $GLOBALS['ecs']->table('order_info') . " AS o ON sbo.order_id = o.order_id" .
                        " WHERE sbo.bill_id = '" . $value['id'] . "'" . $where_order;
...
function get_bill_amount_detail($bill_id = 0, $seller_id = 0, $proportion = 100, $start_time = 0, $end_time = 0, $chargeoff_status = 0, $commission_model = 0){
    ...
    $where_wait_order .= ", " . db_create_in(array(OS_CONFIRMED, OS_SPLITED), "o.order_status") .
        ...
        " AND IF(o.shipping_status = " .SS_RECEIVED. ", " . db_create_in(array(PS_PAYED), "o.pay_status", "NOT") . ", ". db_create_in(array(PS_PAYED), "o.pay_status") . ")" . 
...
function get_alone_goods_rate($order_id = 0, $ru_id = 0, $order = array()){
    ...
    if ($goods_list) {
        ...
        foreach($row['goods'] as $key => $goods){
            ...
            $row[$key]['should_amount'] = number_format($row[$key]['goods_amount'] * $goods['commission_rate'], 2, '.', ''); //应结佣金
            $row[$key]['gain_commission'] = $row[$key]['goods_amount'] - $row[$key]['should_amount']; //收取佣金

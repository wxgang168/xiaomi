...
function goods_list($is_delete, $real_goods=1, $conditions = '')
{
 ...
 /* add by 牛模式(www.niumos.com) start */
       $sql = "SELECT g.goods_id, g.goods_name, g.num_iid, g.goods_thumb, g.click_url, g.original_img, g.commission_num, g.is_taobao, g.shop_title, g.nick, g.goods_weight, NOT LOCATE('http',g.original_img) as r_img,g.goods_name,g.brand_id,g.cat_id,g.model_price, g.model_inventory, g.user_id, g.goods_type, g.goods_sn, g.shop_price, g.is_on_sale, g.is_best, g.is_new, g.is_hot, g.sort_order, g.goods_number, g.integral, " ." g.is_promote " ." FROM " . $GLOBALS['ecs']->table('goods') . " AS g " .                " LEFT JOIN " . $GLOBALS['ecs']->table('goods_cat') . " as gc on g.goods_id = gc.goods_id" . " WHERE $where GROUP BY g.goods_id";
...
/* add by 牛模式(www.niumos.com) start */
        $sql = "SELECT g.goods_id, g.goods_name,g.num_iid, g.goods_thumb, g.click_url, g.original_img, g.commission_num, g.is_taobao, g.shop_title, g.nick, g.goods_weight, NOT LOCATE('http',g.original_img) as r_img, g.user_id, g.brand_id, g.goods_type, g.goods_sn, g.shop_price, g.is_on_sale, " .
                ...
                " LEFT JOIN " . $GLOBALS['ecs']->table('goods_cat') . " as gc on g.goods_id = gc.goods_id" . " WHERE $where" .

    ...
    if (!empty($result['sort'])) {
        foreach ($result['sort'] as $k => $v) {
            ...
            $sql = 'SELECT a.article_id, a.content, a.title, a.add_time, a.description, a.file_url, a.open_type ' .
                    ...
                    ' ORDER BY a.article_type DESC, a.add_time DESC LIMIT 5';
...
elseif ($_REQUEST['act'] == 'edit_cmsgoods') {
    ...
    $result['article_title'] = !empty($_REQUEST['article_title']) ? trim($_REQUEST['article_title']) : '近期热门'; //文章标题
    $result['goods_title'] = !empty($_REQUEST['goods_title']) ? trim($_REQUEST['goods_title']) : '精品推荐'; //商品标题
